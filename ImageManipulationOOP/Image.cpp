...
Image::Image(std::string filename) : image(nullptr), width(0), height(0), filename(filename) {
  load(filename);
}
...
Image& Image::operator=(const Image& old){
  ...
  while(i < old.width ){
    unsigned int j = 0;
    while(j < old.height){
      image[i][j].setGreen(old.image[i][j].getGreen());
      image[i][j].setRed(old.image[i][j].getRed());
      image[i][j].setBlue(old.image[i][j].getBlue());
       ++j;
    }
